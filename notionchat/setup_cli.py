...
from notionchat.bootstrap import (
    Workspace,
    bootstrap_from_cookie,
    list_workspaces_from_cookie_sync,
)
...
def _render_env(values: dict[str, str]) -> str:
    lines = [
        ...
        f"NOTIONCHAT_PORT={values.get('NOTIONCHAT_PORT', '1994')}",
...
def _write_env(path: Path, updates: dict[str, str], *, overwrite: bool) -> None:
    ...
    if path.exists() and not overwrite:
        backup_path = path.with_suffix(".env.bak")
        ...
        for line in existing_lines:
            ...
            if stripped and not stripped.startswith("#") and "=" in stripped:
                ...
                if key in remaining_updates:
                    new_lines.append(f"{key}={remaining_updates.pop(key)}")
                    continue
        ...
        merged = _parse_env_file(path) if path.exists() else {}
        merged.update(updates)
        content = _render_env(merged)
...
async def run_interactive_setup(
    *,
    ...
    if (
        ...
        and not force
        and not yes
        and not _prompt_yes_no(f"{env_path} already exists. Update it?", default=False)
    ):
        write_env = False
    ...
    default_port = port or existing_env.get("NOTIONCHAT_PORT") or "1994"

    if yes:
        resolved_api_key = default_api_key
        resolved_host = default_host
        resolved_port = default_port
        include_cookie = write_cookie_to_env if write_cookie_to_env is not None else False
    else:
        _print()
    ...
    if write_env:
        env_updates = {
            ...
            "NOTIONCHAT_ACCOUNT": str(account_path),
